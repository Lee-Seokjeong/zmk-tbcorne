...
#include "trackball_pim447.h"      // For shared data structures
#include "trackball_pim447_led.h"  // For function declarations

LOG_MODULE_DECLARE(zmk_trackball_pim447, LOG_LEVEL_DBG);
...
int trackball_pim447_set_led(const struct device *dev, pim447_led_t led, uint8_t brightness) {
...
int trackball_pim447_set_leds(const struct device *dev, uint8_t red, uint8_t green, uint8_t blue, uint8_t white) {
    const struct trackball_pim447_config *config = dev->config;
    struct trackball_pim447_data *data = dev->data;
    int ret;
...
void trackball_pim447_led_init(const struct device *dev) {
    struct trackball_pim447_data *data = dev->data;
