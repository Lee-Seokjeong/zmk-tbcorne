/* trackball_pim447.c - Driver for Pimoroni PIM447 Trackball */
...
#include "trackball_pim447_led.h"  // For function declarations

LOG_MODULE_REGISTER(zmk_trackball_pim447, LOG_LEVEL_DBG);
...
static void trackball_pim447_gpio_callback(const struct device *port, struct gpio_callback *cb, gpio_port_pins_t pins);
static int trackball_pim447_enable_interrupt(const struct trackball_pim447_config *config, bool enable);
...
void pim447_enable_sleep(const struct device *dev) {
    ...
    trackball_pim447_set_leds(dev, 0, 0, 0, 0); // Turn off LEDs
...
void pim447_disable_sleep(const struct device *dev) {
    ...
    const struct trackball_pim447_config *config = data->dev->config;
    uint8_t ctrl_reg_value;
    ...
    trackball_pim447_set_leds(dev, 255, 0, 0, 0); // Turn on LEDs
...
static int activity_state_changed_handler(const zmk_event_t *eh) {
    ...
    const struct device *dev = DEVICE_DT_GET(DT_NODELABEL(trackball_pim447));
...
static void trackball_pim447_work_handler(struct k_work *work) {
    struct trackball_pim447_data *data = CONTAINER_OF(work, struct trackball_pim447_data, irq_work);
    const struct trackball_pim447_config *config = data->dev->config;
    ...
    if (speed > 0) {
        ...
        int err;
         ...
         // Set the LEDs
         err = trackball_pim447_set_leds(dev, r, g, b, w);
         if (err) {
...
static void trackball_pim447_gpio_callback(const struct device *port, struct gpio_callback *cb, gpio_port_pins_t pins) {
    struct trackball_pim447_data *data = CONTAINER_OF(cb, struct trackball_pim447_data, int_gpio_cb);
...
/* Enable function */
static int trackball_pim447_enable(const struct device *dev) {
    const struct trackball_pim447_config *config = dev->config;
    struct trackball_pim447_data *data = dev->data;
    int ret;

    LOG_INF("trackball_pim447_enable called");
    ...
    gpio_init_callback(&data->int_gpio_cb, trackball_pim447_gpio_callback, BIT(config->int_gpio.pin));
    ...
    ret = trackball_pim447_enable_interrupt(config, true);
    if (ret) {
        LOG_ERR("Failed to enable interrupt output");
        return ret;
    }

    LOG_INF("trackball_pim447 enabled");

    return 0;
}

/* Disable function */
static int trackball_pim447_disable(const struct device *dev) {
    const struct trackball_pim447_config *config = dev->config;
    struct trackball_pim447_data *data = dev->data;
    int ret;

    LOG_INF("trackball_pim447_disable called");

    /* Disable interrupt output on the trackball */
    ret = trackball_pim447_enable_interrupt(config, false);
    if (ret) {
    ...
    LOG_INF("trackball_pim447 disabled");
...
static int trackball_pim447_init(const struct device *dev) {
    ...
    ret = trackball_pim447_enable(dev);
    if (ret) {
        LOG_ERR("Failed to enable PIM447");
        return ret;
    }

    k_work_init(&data->irq_work, trackball_pim447_work_handler);
...
#define AUTOMOUSE_LAYER (DT_PROP(DT_DRV_INST(0), automouse_layer))
    ...
    static void activate_automouse_layer() {
        ...
        k_timer_start(&automouse_layer_timer, K_MSEC(CONFIG_ZMK_TRACKBALL_PIM447_AUTOMOUSE_TIMEOUT_MS), K_NO_WAIT);
    }
...
static const struct trackball_pim447_config trackball_pim447_config = {
    .i2c = I2C_DT_SPEC_INST_GET(0),
    .int_gpio = GPIO_DT_SPEC_INST_GET(0, int_gpios),
};

static struct trackball_pim447_data pimoroni_pim447_data;

/* Device initialization macro */
DEVICE_DT_INST_DEFINE(0, trackball_pim447_init, NULL, &trackball_pim447_data, &trackball_pim447_config,
